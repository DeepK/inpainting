...
Utrecht_list = sorted(glob(Hospital_dir[2] + "/*"))
...
for nb_file in range(len(Utrecht_list)):

    # Set image path
    T1File = (Utrecht_list[nb_file] + "/pre/T1.nii.gz")
    FLAIRFile = (Utrecht_list[nb_file] + "/pre/FLAIR.nii.gz")
    maskFile = (Utrecht_list[nb_file] + "/wmh.nii.gz")
    ...
    # plt.suptitle("T1 image of patient {}".format(Utrecht_list[nb_file].split('/')[-1]), fontsize=18)
    # start_slice = 20
    # for i in range(0, 6):
    ...
    # Visualize image
    # plt.figure(figsize=(10, 6))
    # plt.suptitle("FLAIR image of patient {}".format(Utrecht_list[nb_file].split('/')[-1]), fontsize=18)
    # start_slice = 20
    # for i in range(0, 6):
    ...
    #
    # plt.figure(figsize=(10, 6))
    # plt.suptitle("Mask on FLAIR image of patient {}".format(Utrecht_list[nb_file].split('/')[-1]), fontsize=18)
    # start_slice = 20
    # for i in range(0, 6):
    #     FLAIRslice = FLAIRVol[i + start_slice]
    ...
    mkd.mkdir('../data')
    mkd.mkdir('../data/Utrecht')
    np.save('../data/Utrecht/img_%04d.npy' % (int(Utrecht_list[nb_file].split('/')[-1])), imageVol)
    np.save('../data/Utrecht/mask_%04d.npy' % (int(Utrecht_list[nb_file].split('/')[-1])), maskVol)

    print('Utrecht Image process {}/{} finished'.format(nb_file, len(Utrecht_list)))
