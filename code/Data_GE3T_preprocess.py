...
GE3T_list = sorted(glob(Hospital_dir[0] + "/*"))
...
for nb_file in range(len(GE3T_list)):

    # Set image path
    T1File = (GE3T_list[nb_file] + "/pre/T1.nii.gz")
    FLAIRFile = (GE3T_list[nb_file] + "/pre/FLAIR.nii.gz")
    maskFile = (GE3T_list[nb_file] + "/wmh.nii.gz")
    ...
    # plt.suptitle("T1 image of patient {}".format(GE3T_list[nb_file].split('/')[-1]), fontsize=18)
    # start_slice = 30
    # for i in range(0, 6):
    ...
    # Visualize image
    # plt.figure(figsize=(10, 6))
    # plt.suptitle("FLAIR image of patient {}".format(GE3T_list[nb_file].split('/')[-1]), fontsize=18)
    # start_slice = 20
    # for i in range(0, 6):
    ...
    #
    # plt.figure(figsize=(10, 6))
    # plt.suptitle("Mask on FLAIR image of patient {}".format(GE3T_list[nb_file].split('/')[-1]), fontsize=18)
    # start_slice = 20
    # for i in range(0, 6):
    #     FLAIRslice = FLAIRVol[i + start_slice]
    ...
    mkd.mkdir('../data')
    mkd.mkdir('../data/GE3T')
    np.save('../data/GE3T/img_%04d.npy' % (int(GE3T_list[nb_file].split('/')[-1])), imageVol)
    np.save('../data/GE3T/mask_%04d.npy' % (int(GE3T_list[nb_file].split('/')[-1])), maskVol)

    print('GE3T Image process {}/{} finished'.format(nb_file, len(GE3T_list)))
